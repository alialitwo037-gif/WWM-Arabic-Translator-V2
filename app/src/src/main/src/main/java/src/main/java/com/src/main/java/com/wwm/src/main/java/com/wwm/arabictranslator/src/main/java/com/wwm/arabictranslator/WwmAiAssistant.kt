...
class WwmAiAssistant(private val context: Context) {
    ...
    fun show() {
        ...
        val ask = Button(context).apply {
            text = "اسأل WWM AI"
        }

        val explain = Button(context).apply {
            text = "فهم النص"
        }

        val translate = Button(context).apply {
            text = "شرح الترجمة"
        }
    ...
    private fun lastOcrText(): String =
        prefs.getString("last_ocr_text", "") ?: ""
    ...
    private fun runAi(
        ...
        executor.execute {
            try {
                ...
                output.post {
                    output.text =
                        "تعذر الاتصال بالذكاء الاصطناعي. استخدمت الوضع المحلي بدلاً منه.\n\n" +
    ...
    private fun localAnswer(
        ...
        val s = source.ifBlank {
            "لا يوجد نص مقروء حالياً."
        }

        return when {
            q.contains("مهم", true) ||
                q.contains("مهمة", true) ||
                q.contains("quest", true) ->
    ...
    private fun localExplain(source: String): String {
        if (source.isBlank()) {
            return "لم يتم التقاط نص حالياً. افتح اللعبة ودع OCR يقرأ الحوار أولاً."
        }
    ...
    private fun localTranslationGuide(source: String): String {
        if (source.isBlank()) {
            return "لا يوجد نص للترجمة حالياً."
        }
    ...
    private fun showAiSettings() {
        ...
        val endpoint = EditText(context).apply {
            ...
            // FIX: use setText instead of assigning String? to EditText.text
            setText(
        ...
        AlertDialog.Builder(context)
            ...
            .setMessage(
                "بدون مفتاح API يعمل المساعد بوضع محلي محدود. " +
                    "عند إضافة مزود متوافق، يرسل التطبيق النص الذي تختاره أنت فقط للتحليل."
            )
            .setView(box)
    ...
    private fun callCompatibleApi(
        ...
        val system =
            "You are WWM AI, a careful Arabic assistant specialized in the mobile game Where Winds Meet. " +
                "Help with translation, dialogue meaning, quests, story context, character names, locations, " +
                "skills and upgrades. Never invent game facts. If the supplied text is insufficient, say so. " +
                "Prefer concise Iraqi-friendly Arabic."
